...
var (
	...
	RequestBodyMaxSize  = 32 * 1024
	ResponseBodyMaxSize = 32 * 1024
...
func NewLogWithConfig(logger *slog.Logger, config LogConfig) fiber.Handler {
	var (
		once       sync.Once
	...
	return func(c *fiber.Ctx) error {
		...
		msg := fmt.Sprintf("[%d] %s %s",
			...
			strings.ToUpper(string(c.Context().Method())),
			path,
		)
